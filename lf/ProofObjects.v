...
Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
  fun (P Q R : Prop) (HPQ: P /\ Q) (HQR: Q /\ R)  => match HPQ with
...
Definition ex_match : forall (A : Type) (P Q : A -> Prop),
  ...
  fun A P Q HPQ HP => 
    match HP with
        | ex_intro _ x Hx => ex_intro (fun x => Q x) x (HPQ x Hx)
    end.
(** [] *)
...
Definition ex_falso_quodlibet' : forall P, False -> P :=
  fun P HFalse => match HFalse with
  end.
(** [] *)
...
Definition eq_cons : forall (X : Type) (h1 h2 : X) (t1 t2 : list X),
    ...
    fun X h1 h2 t1 t2 Hhead Htail => match Hhead with 
      | eq_refl h => match Htail with
...
Definition equality__leibniz_equality_term : forall (X : Type) (x y: X),
    x == y -> forall P : (X -> Prop), P x -> P y :=
    fun X (x y : X) (Heq : eq x y) (P : X -> Prop)  =>
      match Heq with
      | eq_refl x' => (fun y' (HPy' : (P y')) => HPy') x'
      end.  
(** [] *)
...
Definition double_neg : forall P : Prop,
    P -> ~~P :=
    fun (P : Prop) (H : P) (contra : P -> False) => match (contra H) with 
    end.

Definition contradiction_implies_anything : forall P Q : Prop,
    (P /\ ~P) -> Q :=
    fun (P Q : Prop) (contra : P /\ ~P) => match contra with
      | conj HP HNP => match (HNP HP) with
        end
      end. 

Definition de_morgan_not_or : forall P Q : Prop,
    ~ (P \/ Q) -> ~P /\ ~Q :=
    fun (P Q : Prop) (H : ~(P \/ Q))  => conj 
...
Definition curry : forall P Q R : Prop,
    ...
    fun (P Q R : Prop) (H: (P /\ Q) -> R) => 
      fun (HP : P) => fun (HQ : Q ) => H (conj HP HQ). 
...
Proof.
  ...
  intros H P Q.
  apply H. split; intros [H' | H'];[right | left | right | left]; apply H'.
Qed.
...
Proof.
  unfold propositional_extensionality.
  intros H P HP. apply H. split; intros _; [apply HP | apply I].
Qed.
(** [] *)
...
Proof.
  intros H. unfold proof_irrelevance. 
  intros P pf1 pf2.
  assert (P <-> True) as Hiff.
  { split; [ intros _ | intros _]; [ apply I | apply pf1]. }
  apply H in Hiff.
