...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => (negb (n <=? 7)) && (even n)) l.
...
Proof.
  ...
  induction l as [| h t IHt].
    - simpl. reflexivity.
    - simpl. rewrite -> map_distr. rewrite -> IHt. reflexivity.
Qed. 
(** [] *)
...
(* (fold (fun l n => (count l 2) + n) [[1];[];[2;3;2];[4]] 0)
...
Proof.
  ...
  induction l as [| h t IHt].
    - simpl. reflexivity.
    - simpl. unfold fold_length. simpl. rewrite <- IHt. unfold fold_length. reflexivity.
Qed. 
(** [] *)
...
Proof.
  ...
  induction l as [| h t IHt].
    - simpl. reflexivity.
    - simpl. unfold fold_map. simpl. rewrite <- IHt. unfold fold_map. reflexivity.
Qed. 
...
Proof.
  intros X Y Z f p.
  unfold prod_uncurry. unfold prod_curry. destruct p. simpl. reflexivity.
Qed.
(** [] *)
...
*)
(* Induction on l
  if l = [] and n = 0, apparently @nth_error X l n = None;
  if l = h :: t and n = n - 1, and hypotheis: @nth_error X t n - 1 = None:
    Because n != 0 and l != nil, then @nth_error X l n return @nth_error X t n - 1 = None
    []
*)
