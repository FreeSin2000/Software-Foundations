...
(** **** Exercise: 2 stars, advanced, optional (add_comm_informal)
    ...
    - First, suppose [n = 0].  We must show that
      ...
      This follows directly from the add 0 at right side.
    ...
    - Next, suppose [n = S n'], where
      ...
      By the Theorem add_n_sum, this follows from
      ...
      which is immediate from the add n sum.
      _Qed_.
*)
...
Proof.
  intros b.
  rewrite andb_commutative. simpl. reflexivity.
Qed.
...
Proof.
  ...
  induction b as [|n0' IHn0'| n1' IHn1'].
    - simpl. reflexivity.
    - simpl. reflexivity.
    - simpl. rewrite -> IHn1'. rewrite -> add_0_r. rewrite -> add_0_r.
      simpl. 
      rewrite -> plus_n_Sm.  
...
(* Leading Zero in Bin representing is not unique. *)
...
Lemma double_normalize: forall b, double_bin (normalize b) = normalize (double_bin b).
Proof.
  intros b. destruct b.
    - simpl. reflexivity.
    - simpl. reflexivity.
...
Lemma incr_normalize: forall b, incr (normalize b) = normalize (incr b).
Proof.
  intros b. induction b as [|b0' IHb0'| b1' IHb1'].
    - simpl. reflexivity.
    - simpl. reflexivity.
    - simpl. rewrite <- IHb1'. rewrite double_incr_bin. reflexivity.
Qed.

Lemma double_bin_to_nat: forall n: nat, nat_to_bin (n + n) = normalize (B0 (nat_to_bin n)).
Proof.
  intros n. induction n as [|n' IHn'].
    - simpl. reflexivity.
    - simpl. rewrite <- plus_n_Sm. simpl. rewrite -> IHn'. simpl. 
      rewrite -> double_normalize. rewrite -> double_normalize.
      rewrite -> incr_normalize. rewrite -> incr_normalize.
      rewrite <- double_incr_bin. reflexivity.
Qed.

Lemma normalize_power: forall b: bin, normalize b = normalize (normalize b).
Proof.
  intros b. induction b as [|b0' IHb0'| b1' IHb1'].
    - simpl. reflexivity.
    - simpl. rewrite <- double_normalize. rewrite <- IHb0'. reflexivity.
    - simpl. rewrite <- incr_normalize. rewrite -> double_normalize.
      rewrite <- double_normalize. rewrite <- double_normalize.
...
Proof.
  intros b. induction b as [|b0' IHb0'| b1' IHb1'].
    ...
    - simpl. rewrite add_0_r.
      ...
      rewrite -> IHb0'. simpl. rewrite <- normalize_power. reflexivity.
    - simpl. rewrite add_0_r.
      rewrite -> double_bin_to_nat. simpl. rewrite -> IHb1'. rewrite <- normalize_power. reflexivity.
Qed. 
